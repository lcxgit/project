...
document.write("<scr"+"ipt src='js/skel.js' type='text\/javascript' charset='utf-8'></scr"+"ipt>");
...
$(document).ready(function() {
    (function($) {
        ...
        $(function() {
            ...
            // Keyboard shortcuts.
            if (settings.keyboardShortcuts.enabled)(function() {

                $wrapper

                // Prevent keystrokes inside excluded elements from bubbling.
                .on('keypress keyup keydown', settings.excludeSelector,
                function(event) {
            ...
            // Scroll zones.
            if (settings.scrollZones.enabled)(function() {
                ...
                $left.css('left', '0').on('mouseenter',
                function(event) {
                    activate( - 1);
                });

                $right.css('right', '0').on('mouseenter',
                function(event) {
                    activate(1);
                });

                $wrapper.on('---pauseScrollZone',
                function(event) {
            ...
            // Dragging.
            if (settings.dragging.enabled)(function() {

                var dragging = false,
                dragged = false,
                distance = 0,
                startScroll, momentumIntervalId, velocityIntervalId, startX, currentX, previousX, velocity, direction;

                $wrapper

                // Prevent image drag and drop.
                .on('mouseup mousemove mousedown', '.image, img',
                function(event) {
                    event.preventDefault();
                })

                // Prevent mouse events inside excluded elements from bubbling.
                .on('mouseup mousemove mousedown', settings.excludeSelector,
                function(event) {
            ...
            // Link scroll.
            $wrapper.on('mousedown mouseup', 'a[href^="#"]',
            function(event) {
            ...
            }).on('mouseup mousedown mousemove', '.modal',
            function(event) {
