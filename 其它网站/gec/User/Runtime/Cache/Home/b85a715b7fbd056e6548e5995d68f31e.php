        ...
        <div class="col-md-13">
            <div class="box box-info">
                <div class="box-header with-border">
                    <!-- <h3 class="box-title" style="color: #333;/* background: #05b6a3; */ text-align: center;border-bottom: 1px solid #ddd;border-top: 1px solid #ddd;font-size: 18px;padding: 10px 0;">价格走势图</h3> -->
                </div>
				    <div id="myhend">
                    <div>
                        <p>幅:&nbsp;<?php echo ($fu); ?>%</p>
...
<!-- <div class="box-body chart-responsive" style="background-image: url('/Public/user2/images/kx.png');"> -->
	...
	<div style="width: 90%;margin-left: 5%;margin-top: 20px;overflow:hidden;border-radius:5px">
		<p id="qiugou_list"onclick='showhidediv("qiugou")'style="float: left;width: 50%;text-align: center;background-color: #05B6A3;height: 30px;line-height: 30px;color:#fff;border-bottom-left-radius: 5px;border-top-left-radius: 5px">买入GEC</p>
 ...
 style="width:60%"/>
		...
		<button type="submit" id="mmm" style="float:right;width:33%;margin-top:-20px;background-color: #05B6A3;height: 30px;line-height: 30px;background-color: #05B6A3;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">买入</button>
	</form>
	<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr style="margin-bottom:100px">
				...
				<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
					<button style="background-color:#05B6A3;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnchushou">等待中</button>
				</td><?php endif; ?>
	...
	<form id="qiugou" class="nnform" style="margin-top: 25px" action="/Index.php/Home/Info/cslkb" method= "POST" onsubmit="return false">
		...
		<?php if($level > 1): ?><input type="text" name="zdjy" id="mmp" placeholder="请输入指定交易的用户"style="width:60%"/><?php endif; ?>
		<button type="submit" id="nnn" style="float:right;width:33%;margin-top:-20px;background-color: #05B6A3;height: 30px;line-height: 30px;background-color: #05B6A3;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">卖出</button>
	</form>
		<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
			<thead>
			<tr style="width:100%;">
			...
			<?php if(is_array($lists)): foreach($lists as $key=>$v): ?><tr style="margin-bottom:100px">
					...
					<td class="range iconfont" style="padding:10px 0;text-align:center;font-size:12px">
						<?php if($v["p_level"] == 0): ?>未激活<?php endif; ?>
						<?php if($v["p_level"] == 1): ?>GEC矿工<?php endif; ?>
						<?php if($v["p_level"] == 2): ?>公会会长<?php endif; ?>
						<?php if($v["p_level"] == 3): ?>创业大使<?php endif; ?>
						<?php if($v["p_level"] == 4): ?>环保大使<?php endif; ?>
						<?php if($v["p_level"] == 5): ?>国际大使<?php endif; ?>
					...
					<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color:#05B6A3;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnsc">等待中</button>
						</td><?php endif; ?>
					<?php if($v["zt"] == 1): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color:#666;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" >交易中</button>
						</td><?php endif; ?>
...
function btnchushou (obj) {
        ...
        $.ajax({
            url:"<?php echo U('/Home/Info/huilv');?>",
            type:"get",
            ...
            success:function(data){
                ...
                $.confirm("USD："+huilv+"<br>"+"RMB："+data.rmb+"<br>"+"BTC："+data.btc,"确定提交？",function(){
                    $.ajax({
                        url:"<?php echo U('/Home/Info/chushou');?>",
                        type:"get",
...
<script type="text/javascript">
    ...
    $.ajax({
        url:"<?php echo U('/Home/Ajax/getgp');?>",
        type:"post",
        data:null,
        dataType:"json",
        success:function(data){
    ...
    $("#mynav a").eq(1).bind("click",function(){
        $.ajax({
            url:"<?php echo U('/Home/Ajax/getgps');?>",
            type:"post",
            data:null,
            dataType:"json",
            success:function(data){
