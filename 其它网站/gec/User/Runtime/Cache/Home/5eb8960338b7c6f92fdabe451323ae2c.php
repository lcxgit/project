...
<ul class="dd_list"style="margin-bottom:80px;">
	<?php if(is_array($list)): foreach($list as $key=>$v): ?><li style="position:relative;">
			...
			<div style="width:62%;display:inline-block;">
				...
				<P style="font-size:12px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;width:100%;height:20px;line-height:30px">运行周期：<?php echo ($v["yxzq"]); ?>小时&nbsp;&nbsp;&nbsp;&nbsp;<span>产量/小时：<?php echo ($v["lixi"]); ?></span></P>	
				<p style="font-size:12px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;width:100%;height:20px;line-height:30px">矿机编号：<?php echo ($v["kjbh"]); ?></p>
				<p style="font-size:12px;height:20px;line-height:30px">矿机状态：<?php if($v["zt"] == 0): ?>未使用<?php endif; ?>
							<?php if($v["zt"] == 1): ?>正在运行<?php endif; ?>
								<?php if($v["zt"] == 2): ?>已经结束<?php endif; ?>
								
				</p>		
			</div>
	<?php if($v["zt"] == 0): ?><p style="height:70px;line-height:70px;text-align:right;width:15%; display:inline-block;"><a href="/Index.php/Home/Info/wakuang/?id=<?php echo ($v["id"]); ?>"style=" height:30px;line-height:30px;color: #fff;margin-top: 0px;display:block;position:absolute;right:10px;top:50%;margin-top: -15px;font-size: 16px;padding: 3px 5px;background-color: #fe9625;border: 0px solid #fff;border-radius: 4px;">运行</a></p><?php endif; ?>
