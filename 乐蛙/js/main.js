...
var lanren = {
    ...
    init: function () {
        var lis = $(".product_list .hidden .product_figure");
        $(".product_list .list_content").html("");
        for (var n = 0; n < lanren._default; n++) {
            lis.eq(n).appendTo(".product_list .list_content");
        }
        $(".product_list .list_content img").each(function () {
    ...
    loadMore: function () {
        var mLis = $(".product_list .list_content .product_figure").length;
        for (var i = 0; i < lanren._loading; i++) {
            var target = _content_news.shift();
            if (!target) {
                $('.product_list .news_list_more').html("<p>全部加载完毕...</p>");
                break;
            }
            $(".product_list .list_content").append(target);
            $(".product_list .list_content img").eq(mLis + i).each(function () {
                $(this).attr('src', $(this).attr('realSrc'));
            });
        }
    }
