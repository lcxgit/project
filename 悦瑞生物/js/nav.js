...
// 导航栏
(function() {
	var triggerBttn = document.getElementById( 'trigger_a' ),
		menu_main = document.querySelector( '.menu_main' ),
		closeBttn = menu_main.querySelector( '.btn_close' );
		transEndEventNames = {
	...
	function toggleOverlay() {
		if( classie.has( menu_main, 'open' ) ) {
			classie.remove( menu_main, 'open' );
			classie.add( menu_main, 'close' );
			var onEndTransitionFn = function( ev ) {
				if( support.transitions ) {
					if( ev.propertyName !== 'visibility' ) return;
					this.removeEventListener( transEndEventName, onEndTransitionFn );
				}
				classie.remove( menu_main, 'close' );
			};
			if( support.transitions ) {
				menu_main.addEventListener( transEndEventName, onEndTransitionFn );
			}
			else {
				onEndTransitionFn();
			}
		}
		else if( !classie.has( menu_main, 'close' ) ) {
			classie.add( menu_main, 'open' );
		}
	}
